...
#' @description mastermix is a linear deconvolution procedure for STR DNA mixtures.
#' @export
#' @usage mastermix(mixData, nC, method = 'greedy', model = 'general', eps = NULL, locsel_Mix = NULL, refData = NULL, locsel_Ref = NULL, condOrder = NULL, zeroMx = FALSE, threshT = 50)
...
#' @param locsel_Mix Selected loci in mixData to deconvolve. locsel_Mix=NULL; accepts all loci.
#' @param refData Reference objects with list element adata[[s]][[i]]. The list element has reference-list with list-element 's' having a loci-list with list-element 'i storing qualitative data.
#' @param locsel_Ref matrix of boolean specifying conditional loci (row) for each reference (column).locsel_Ref=NULL; accepts all loci.
...
#' deconv1 = mastermix(mixData,nC,method='greedy',model='gen',zeroMx=FALSE)
#' deconv2 = mastermix(mixData,nC,method='peeloff',model='gen',eps=50,zeroMx=FALSE)
#' print(deconv1$res2)
...
#' deconv1 = mastermix(mixData,nC,method="greedy",model="gen",eps=NULL,locsel_Mix,refData,locsel_Ref,condOrder,zeroMx=FALSE)
#' deconv2 = mastermix(mixData,nC,method="peeloff",model="gen",eps=50,locsel_Mix,refData,locsel_Ref,condOrder,zeroMx=FALSE)
#' print(deconv1$res2)
#' print(deconv2$res2[1:nC,])
...
mastermix = function(mixData,nC,method='greedy',model='general',eps=NULL,locsel_Mix=NULL, refData=NULL,locsel_Ref=NULL,condOrder=NULL,zeroMx=FALSE,threshT=50) {
 require(MASS)
 require(gtools)
 #scaling of height is incorporated into model as Yi+.
 #ERROR HANDLE:
 if(is.null(mixData$adata) | is.null(mixData$hdata)) { print('Missing data'); return(NULL) }
 ...
 if(!is.null(refData$adata)) {
  nR = length(refData$adata) #number of references
  for(k in 1:nR) {
   if(length(refData$adata[[k]])!=nL) { print('Wrong loci length in refData'); return(NULL) }
  }
 }
 if(!is.null(locsel_Mix) && length(mixData$adata)!=length(locsel_Mix)) { print('Wrong data format in locsel_Mix'); return(NULL) }
 if(!is.null(locsel_Ref) && length(refData$adata)!=ncol(locsel_Ref)) { print('Wrong data format in locsel_Ref'); return(NULL) }
 if(!is.null(condOrder) && !is.null(refData) && length(refData$adata)!=length(condOrder))  { print('Wrong length condOrder given'); return(NULL) }


 #insert lociname if not already given
 if(is.null(mixData$lociname)) mixData$lociname = paste('Loci',1:nL,sep='')
 ...
 if(!is.null(condOrder)) {
  ...
  for(k in 1:length(condOrder)) {
   for(i in 1:nL) {
    if(condOrder[k]==0 | length(refData2$adata[[k]][[i]])==0) next
    aref = refData2$adata[[k]][[i]]
 ...
 getContr_combs <- function(Alist,i,nC,symmetry=FALSE,refs=NULL,condOrder=NULL) {
  ...
  if(any(Alist[[i]]%in%c('X','Y'))) singleCombs = singleCombs[!rowSums(singleCombs==1)==2,]  #if amel: REMOVE YY-combination!
  ...
  if(!is.null(refs)) { #if references given
   nRefs = length(refs$adata) #number of references selected from import-frame
   ...
   for(k in 1:nRefs) { #for each reference we want to keep only combinations where these are included
    ...
    if(length(refs$adata[[k]][[i]])==0) next #continue if no reference there
    if(!all(refs$adata[[k]][[i]]%in%Alist[[i]])) next #require both alleles to be in Alist
    CC = toAscii (which(Alist[[i]]%in%refs$adata[[k]][[i]])) #get combinations 
...
#  Yhat <- X%*%mhat + O #predicted Y-respons from model.
 ...
 # }
  ...
  mhat <- c(mhat,1-sum(mhat)) #estimate of mhat
  return(list(mhat=mhat,MD = MD,Yhat=Yhat))
 }
...
####################################################################################################
 ...
 getCombs <- function(deconvlist) {
  ...
  combs <- cbind(combs, round(deconvlist$pList$Mx,3) ,round(deconvlist$pList$MD,3) )
  addcolname <- c(paste('Mx',1:nC,sep='_'),'MD')
  rownames(combs) <- 1:nJ
  colnames(combs) <- c(ln,addcolname) #paste(c(t(replicate(2,ln))),c('_1','_2'),sep='')
  return(combs)
 }
 ...
 deconvlist$res1 = getCombs(deconvlist)
 deconvlist$res2 = getCombs2(deconvlist)
